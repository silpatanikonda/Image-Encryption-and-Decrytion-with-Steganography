...
class UnhideTextApp:
    def __init__(self, parent):
        ...
        parent.add(self.page, text="Unhiding Text App")

        # Variables
        self.stegano_image_path = StringVar()
        
        # UI Elements
        tk.Label(self.page, text="Image Steganalysis", font=("Helvetica", 16, "bold")).grid(pady=10,row=0)

        tk.Label(self.page, text="Select Steganographic Image:").grid(pady=10,row=1)
        tk.Entry(self.page, textvariable=self.stegano_image_path, state="readonly", width=40).grid(padx=5, pady=5,row=2)
        tk.Button(self.page, text="Browse", command=self.browse_stegano_image).grid(pady=10,row=3)

        tk.Button(self.page, text="Verify Image", command=self.verify_image).grid(pady=10,row=4)

        # Hidden Text Display Area
        tk.Label(self.page, text="Hidden Text:").grid(pady=10,row=5)
        self.hidden_text_display = tk.Entry(self.page, width=30, state="readonly")
        self.hidden_text_display.grid(pady=10,row=6)

        # File Name Display Area
        self.file_name_display = tk.Label(self.page, text="")
        self.file_name_display.grid(pady=5,row=7,columnspan=2,column=0)

        # Image Preview Area
        self.image_preview = tk.Label(self.page)
        self.image_preview.grid(pady=5,row=9,columnspan=2)

        # Close Button
        tk.Button(self.page, text="Close", command=self.close_window).grid(pady=5, row=10, column=1,sticky="e")

        # Refresh Button
        tk.Button(self.page, text="Refresh", command=self.refresh_fields).grid(pady=5,row=10,column=0,sticky="w")
    ...
    def verify_image(self):
        ...
        messagebox.showinfo("Processing","Steganalysis is in Process....\nPlease Wait")
        ...
        try:
            ...
            if hidden_text:
                ...
                self.file_name_display.config(text=f"Selected Image:\nName: {file_name}\nPath: {stegano_image_path}")
                
                # Image Preview Text
                self.image_preview_text = tk.Label(self.page, text="Image Preview:")
                self.image_preview_text.grid(pady=5,row=8,columnspan=2,column=0)
    ...
    def display_image_preview(self, image):
        ...
        image = image.resize((220, 220), Image.ANTIALIAS if hasattr(Image, "ANTIALIAS") else Image.NEAREST)
...
class MainApp:
    def __init__(self, root):
        ...
        self.root.geometry("460x700+100+50")
